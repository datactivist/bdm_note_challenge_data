...
La ville du Taillan Médoc a publié un jeu de données sur les [délibérations](https://opendata.bordeaux-metropole.fr/explore/dataset/let_deliberations_2023/information/). Ce jeu de données à la particularité de faire partie des jeux de données du socle commun des données locales (SCDL). Ce "label" identifie les données prioritaires et normalisées que doivent produire en priorité les collectivités locales.
...
La visualisation en treemap proposée par Open Data Soft montre ses limites.
...
Ce cas montre qu'il peut être intéressant parfois de réaliser la visualisation à l'aide d'un autre outil et modifiant légèrement le jeu de données, en séparant les matières et sous-matières dans notre cas.
...
Ce jeu de données commence en mars 2023. Il sera intéressant de le mettre à jour après chaque conseil municipal. Le cercle central pourra évoluer pour ajouter les années afin de comparer dans les temps la répartition du nombre de délibérations par matière.
